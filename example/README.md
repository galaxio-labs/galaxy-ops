...
- **`prj import` 要求系统已解析变量**：导入流程依赖 `sys/resolved_vars.yml`（由 `gops sys update` 解析模块并合并变量后生成）。若导入一个从未 `update` 过的系统，会明确报错：`系统变量未解析：缺少 .../sys/resolved_vars.yml。请先在该系统上执行 gops sys update 解析变量，再打包导入`。推荐直接使用 `gops sys package`（内部先 update 再打包），避免手动打包漏掉 update。
